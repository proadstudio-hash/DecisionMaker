...
// Problem Families (14 families) - V5 with consolidated CAPEX
...
const app = {
  ...
  selectFamily(familyId) {
    ...
    // Load criteria (core DISABLED by default + extra ENABLED)
    state.criteria = [
      ...CORE_CRITERIA.map(c => ({...c, isCore: true, isIncluded: false, locked: false, source: 'core'})),
      ...family.extraCriteria.map(c => ({...c, isCore: false, isIncluded: true, locked: false, source: 'extra'}))
    ];
  ...
  loadExample() {
    ...
    // Initialize performance matrix with mid values (only for enabled criteria)
  ...
  // Step 3: Enhanced Criteria
  renderCriteriaEnhanced() {
    this.updateFamilyBadges();
    this.renderCoreCriteria();
    this.renderExtraCriteria();
    this.updateWeightSumEnhanced();
  },

  renderCoreCriteria() {
    const list = document.getElementById('core-criteria-list');
    list.innerHTML = '';
    
    const coreCriteria = state.criteria.filter(c => c.isCore);
    coreCriteria.forEach((crit, index) => {
      const item = document.createElement('div');
      item.className = `crit-item-enhanced core`;
  ...
  renderExtraCriteria() {
    const list = document.getElementById('extra-criteria-list');
    const label = document.getElementById('family-specific-label');
    
    if (state.selectedFamily) {
      label.textContent = `(${state.selectedFamily.nome})`;
    }
    
    list.innerHTML = '';
    
    const extraCriteria = state.criteria.filter(c => !c.isCore && c.source === 'extra');
    extraCriteria.forEach((crit, index) => {
      const item = document.createElement('div');
    ...
    modal.innerHTML = `
      <div style="background: var(--color-surface); padding: 32px; border-radius: var(--radius-lg); max-width: 500px; width: 90%;">
        ...
        <div class="form-group">
          ...
          <input type="text" id="custom-crit-name" class="form-control" placeholder="es. Impatto ambientale, Flessibilità..." maxlength="100">
  ...
  resetToDefaults() {
    ...
    // Reset to original weights and states (core DISABLED, extra ENABLED)
    const coreCriteria = CORE_CRITERIA;
    const extraCriteria = state.selectedFamily.extraCriteria;
    
    state.criteria = [
      ...coreCriteria.map(c => ({...c, isCore: true, isIncluded: false, locked: false, source: 'core'})),
      ...extraCriteria.map(c => ({...c, isCore: false, isIncluded: true, locked: false, source: 'extra'}))
    ];
  ...
  calculateResults() {
    try {
      console.log("=== V8.4 CALCULATION STARTED (5 METHODS - COST INVERSION FIXED) ===");
      ...
      console.log("=== V8.4 CALCULATION COMPLETE - COST INVERSION VERIFIED ===");
  ...
  displayRankingsTable() {
    ...
    state.results.rankings.forEach((rank, index) => {
      ...
      row.innerHTML = `
        ...
        <td style="padding: 10px; text-align: center;"><strong style="font-size: 16px; color: #667eea;">${rank.score.toFixed(1)}</strong></td>
        <td style="padding: 10px; text-align: center;"><span style="background: #e0e0e0; padding: 4px 8px; border-radius: 4px;">${confidencePct}%</span></td>
      `;
  ...
  displayDetailedExplanation() {
    ...
    html += '<h4>Metodologia V8.4 (Cost Inversion Fixed)</h4>';
    ...
    html += '</ul>';
    html += '<strong>✅ Correzione V8.4 - Inversione Costi:</strong>';
    html += '<ul style="font-size: 12px; color: var(--color-text-secondary);">';
    html += '<li>✓ Criteri COSTO: invertiti (10-valore) DENTRO normalizeMatrix()</li>';
    ...
    html += '<li>✓ Verifica: costo basso (es. 2/10) → dopo inversione (8/10) → score alto ✓</li>';
    html += '</ul></p>';
